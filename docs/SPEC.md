...
|Model header|§7: «все параметры (endpoints, rpcid, model header, ключи WIZ) живут в `gemini-web.json`»|Шаблоны `modelHeader`, `batchModelHeader`, `sessionHeader` читаются из `gemini-web.json`|Без этого обещание R1 было не выполнено: заголовок был зашит в Swift, и ошибка 1052 требовала пересборки с переустановкой|
...
### 13.4 Вход в Google из WebView

Google прямо перечисляет, из каких браузеров не пускает вход
(<https://support.google.com/accounts/answer/7675428>): среди прочего — «Are being
controlled through software automation rather than a human» и «Are embedded in a
different application». Штатный User-Agent `WKWebView` — ровно эта подпись: в нём
есть `Mobile/…`, но нет ни `Version/…`, ни `Safari/…`. Поэтому и транспорт, и
экран входа подставляют User-Agent мобильного Safari для текущей версии iOS
(`SafariUserAgent`), и это единственное, что отличает встроенный WebView от
Safari: движок тот же, запрос делает сама страница, а не нативный клиент.

Если Google всё же покажет «This browser or app may not be secure», это не
лечится на стороне приложения: тогда остаётся вариант «войти в Safari и
пользоваться Gemini оттуда», а приложение переводить не сможет — куки Safari и
куки приложения на iOS не пересекаются. Риск зафиксирован, но не подтверждён:
проверить его можно только на устройстве.

### 13.5 Токен `SNlM0e` стал необязательным (2026)

**Это была причина того, что вход проходил, а приложение считало, что его нет.**

С февраля–апреля 2026 Google перестал надёжно отдавать `SNlM0e` в HTML страницы
`/app`, оставив `cfb2h` и `FdrFJe`. Вся поддерживаемая экосистема это уже учла:
HanaokaYuzu/Gemini-API выпустил «allow init without SNlM0e token» (v1.20.0), затем
v2.0.0 и v2.1.0 с guest-режимом, а gpt4free просто не отправляет поле, когда токена
нет. Признак «нет `SNlM0e` ⇒ не вошёл» больше неверен, причём в обе стороны: у
страницы входа есть **свой** `SNlM0e` и `FdrFJe`, поэтому их наличие тоже ничего не
доказывает.

**Эта же ошибка сидела ещё в двух местах, и оба нашлись только сверкой с живой
страницей.** Скрипт извлечения внутри себя определял пригодность находки так:
`complete = at && bl && sid`, а запасной вариант — `partial = at`. То есть требование
токена стояло не только в решении о входе, но и в самом разборе: на странице без
`SNlM0e` стратегии «нашлось `bl` и `f.sid`, но без токена» не существовало, находка
выбрасывалась, и приложение получало пустые поля. Правка одного лишь решения о входе
не помогла бы — симптом остался бы прежним.

Теперь критерий один и тот же везде — то, без чего запрос не собрать: `bl` и `f.sid`.
Токен не участвует в критерии ни в одну сторону. Три сценария закреплены в
`tools/test-wiz-extraction.mjs`: страница без токена (регресс), токен без `bl`/`f.sid`
(не сессия) и страница без ничего.

Проверка на живой странице: `node tools/check-live-session.mjs` тянет
`gemini.google.com/app` тем же мобильным User-Agent и прогоняет тот же скрипт извлечения,
взятый прямо из `GeminiWebTransport.swift`, поэтому они не могут разойтись. На
2026-09-12 страница отдаёт 127 ключей `WIZ_global_data`, `cfb2h` и `FdrFJe`, и
**ни одного вхождения `SNlM0e`** — извлечение возвращает `bl` и `sid`, `at` пуст.
Этот инструмент не в CI: он зависит от живой страницы Google, и его падение означало бы
не поломку сборки, а изменение на стороне Google.

Отсюда правила, принятые в коде:

|Вопрос|Ответ|
|---|---|
|Можно ли отправлять запрос|Да, если есть `bl` (`cfb2h`) и `f.sid` (`FdrFJe`). `at` уходит пустым|
|Вошел ли пользователь|Да, если есть `bl` и `f.sid`, страница пришла с хоста `gemini.google.com`, а в **своём** хранилище WebView лежит `__Secure-1PSID`; окончательно подтверждает RPC `otAQ7b`. Решение вынесено в `SessionEvaluator` (ядро) и покрыто табличными тестами — в приложении этот выбор был самой ответственной веткой во всём продукте и при этом не проверялся ничем, кроме успешной компиляции|
|Чего нельзя делать|Считать признаком входа наличие `TL` `SNlM0e` и читать параметры со страницы, которая не является приложением|

Коды статуса аккаунта теперь разобраны полностью, с текстом под каждый:
1000 доступен, 1014 временно ограничен, 1016 не авторизован, 1021 отклонён,
1033 не прошёл проверку, 1040/1042 условия использования, 1054/1057 родительский
контроль, 1060 регион; неизвестный код трактуется как «отклонён», а не как «всё
хорошо».

Дополнительно уточнены роли UUID: последний элемент model header — это
**клиентский** id сессии (на запуск приложения), а payload-слот 59 и заголовок
`x-goog-ext-525005358-jspb` несут **свежий UUID на каждый запрос**.

Источники: <https://github.com/HanaokaYuzu/Gemini-API/blob/master/src/gemini_webapi/utils/get_access_token.py>,
<https://github.com/HanaokaYuzu/Gemini-API/issues/297>,
<https://github.com/xtekky/gpt4free/blob/main/g4f/Provider/needs_auth/Gemini.py>,
<https://github.com/teng-lin/notebooklm-py/blob/main/src/notebooklm/_auth/extraction.py>
(описывает ловушку с токеном страницы входа).

### 13.6 Модели: истина — аккаунт

Пресеты в `gemini-web.json` — стартовая точка на случай, если RPC `otAQ7b` ничего
не вернул. Как только RPC отвечает, список берётся **только** из него, целиком:
подставлять рядом устаревшие пресеты нельзя, иначе в выборе модели окажутся
записи, которых у аккаунта нет.

Названия в пресетах приведены к тому, что аккаунт реально отдаёт в сентябре 2026:
`56fdd199312815e2` — **3.8 Flash**, `e6fa609c3fa255c0` — 3.1 Pro,
`8c46e95b1a07cecc` — 3.5 Flash-Lite. Утверждение из §1 («Gemini 3.8 flash не
существует») было ошибочным: нумерация действительно дошла до 3.8, и это тот же
id, что стоял в плане как безымянный «Flash подписки».
Источник: <https://github.com/yeahhe365/Gemini-Nexus/blob/main/docs/gemini-web-reverse.md> (проверено 2026-09-03).

### 13.7 Тесты app-слоя

`xcodebuild test` в CI прогоняет и app-таргет, так что у очереди перевода —
самого рискованного компонента — есть настоящие тесты
(`AppTests/TranslationQueueTests.swift`, 13 штук): успешный прогон, ретрай с
последующим успехом, постоянная ошибка, ожидание лимита, «нет связи» без
расхода попыток, остановка на `.waitingAuth` и возобновление, пауза
пользователя, пауза из-за ухода приложения в фон, правила восстановления из
`state.json`, удаление книги. Тайминги (бэкофф, ожидание лимита, ожидание
связи) внедряются, поэтому тесты идут миллисекунды, а не минуты.

Пауза раньше не имела пути назад: `scenePhase != .active` останавливал очередь,
в UI оставалась только мёртвая кнопка «Пауза», а `resumeFromStoredState`
игнорировал `.paused` — то есть любое переключение приложения навсегда
останавливало перевод. Теперь у паузы есть причина: пользовательская
сохраняется как `.paused` и снимается только явным возобновлением, а пауза от
ухода в фон оставляет состояние «не завершено» и возобновляется при возврате
...
- **Что удалось проверить без входа в аккаунт** (2026-09-12). `gemini.google.com/app`
  отдаёт 200, и в живой разметке присутствуют: базовые пути `/_/BardChatUi/…`,
  ключи WIZ `cfb2h`, `FdrFJe`, `TuX5cc` — и все четыре id моделей из
  `gemini-web.json` (`56fdd199312815e2`, `e6fa609c3fa255c0`, `8c46e95b1a07cecc`,
  `fbb127bbb056c959`). Ключа `SNlM0e` в анонимной странице нет, что совпадает с
  логикой «нет `at` → нужен вход». Кроме этих четырёх в шелле упоминается ещё
  около двадцати id-моделей, то есть список из RPC действительно нужен как
  источник истины, а пресеты — только стартовая точка.
  Не подтверждено: rpcid `otAQ7b`/`jSf9Qc`/`qpEbW`, имена `x-goog-ext-…-jspb` и
  индексы payload. Реальный бандл приложения подтягивается обфусцированным
  загрузчиком (`/_/mss/boq-bard-web/_/js/k=boq-bard-web.BardChatUi…/m=_b?wli=…`),
  и без manifest его модули не перечислить; запущенный отдельным профилем Chrome
  тоже не помог — CDP на `127.0.0.1` из WSL недостижим (разные сетевые
  неймспейсы), а relay к браузеру пользователя не настроен и висит.
  Смотреть эти значения на устройстве: «Диагностика → Перечитать параметры» —
  и, если разойдётся, править `Documents/Config/gemini-web.json` без пересборки.
